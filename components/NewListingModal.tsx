...
export default function NewListingModal({ profileId, onClose }: { profileId: string; onClose: () => void }) {
      ...
      <div onClick={(e) => e.stopPropagation()} className="drawer-panel" style={{ width: "100%", maxWidth: "600px", background: "var(--cream)", height: "100%", overflowY: "auto", boxShadow: "-4px 0 32px oklch(0% 0 0 / 0.2)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "24px 28px", borderBottom: "1px solid var(--sand)", position: "sticky", top: 0, background: "var(--cream)", zIndex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            {step > 1 && <button type="button" onClick={() => setStep(step - 1)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-light)", fontSize: "13px", padding: 0, fontFamily: "Manrope, var(--font-manrope)" }}>← Back</button>}
            <h2 style={{ fontFamily: "'Bricolage Grotesque', var(--font-bricolage)", fontSize: "20px", fontWeight: 700, color: "var(--text)", margin: 0 }}>New Listing</h2>
          </div>
        ...
        <div style={{ padding: "24px 28px" }}>
          ...
          <div style={{ display: "flex", gap: "6px", marginBottom: "16px" }}>
            {[0,1,2].map((i) => <div key={i} style={{ flex: 1, height: "3px", borderRadius: "2px", background: i < step ? "var(--rust)" : "oklch(87% 0.028 76)", transition: "background 0.3s" }} />)}
          </div>
          <p style={{ fontSize: "13px", color: "var(--text-light)", marginBottom: "24px" }}>Step {step} of 3 — <strong style={{ color: "var(--text)" }}>{stepTitles[step - 1]}</strong></p>

          <form onSubmit={goNext} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            {step === 1 && <>
              <F label="Title" required error={errors1.title}><input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Fractal Geometry Hoodie" maxLength={80} style={{ ...IS, borderColor: errors1.title ? "var(--rust)" : undefined }} /><p style={{ fontSize: "11px", color: "var(--text-light)", margin: 0 }}>{title.length}/80</p></F>
            ...
            {step === 2 && <>
              ...
              <button type="submit" style={{ padding: "14px", background: "var(--rust)", color: "white", border: "none", borderRadius: "8px", fontSize: "15px", fontWeight: 700, cursor: "pointer", fontFamily: "Manrope, var(--font-manrope)", marginTop: "8px" }}>Continue — Review →</button>
            </>}

            {step === 3 && <>
              {images.length > 0 && <div style={{ borderRadius: "12px", overflow: "hidden", border: "1px solid var(--sand)", maxHeight: "280px" }}><img src={images[0]} alt="Cover" style={{ width: "100%", height: "280px", objectFit: "cover", display: "block" }} /></div>}
