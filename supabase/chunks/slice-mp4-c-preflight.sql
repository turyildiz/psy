...
checks(name,ok,detail) as (
 ...
 ('preflight_policy_exact',(m.policy_n=17 and m.policy_h='88b11cfcc7f9dc11e40259cfc5c67dd3'),'policy manifest drift'),
...
select 'SLICE_MP4_C_PREFLIGHT' package,case when all_ok then 'GO' else 'STOP' end verdict,passed,total,findings,'Exact six-table catalog pins come from the 2026-08-15 MP4-C live-evidence bundle; active-authority helper pins come from applied MP4-A after-state. No row aggregates were admitted. Account notification preference is outside these six tables and is untouched, not independently repinned here.'::text boundary from summary;
rollback;
