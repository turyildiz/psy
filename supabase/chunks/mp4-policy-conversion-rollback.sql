...
actual_functions as (
  ...
  if drift then raise exception 'MP-4 rollback precondition guard failed: policy manifest drift'; end if;
...
actual_functions as (
  ...
  if drift then raise exception 'MP-4 rollback precondition guard failed: function manifest drift'; end if;
end;
$guard$;
...
drop policy "Owners can read own private listings" on public.listings;
...
using (
  conversation_id in (
    ...
    where conversations.buyer_profile_id in (
      select profiles.id from public.profiles where profiles.user_id = auth.uid()
    ) or conversations.seller_profile_id in (
      select profiles.id from public.profiles where profiles.user_id = auth.uid()
    )
  )
);
...
drop function public.current_user_owns_unsuspended_profile(uuid);
...
actual_functions as (
  ...
  select exists ((select * from actual_policies where (table_name,policy_name) in (select table_name,policy_name from expected_policies) except select * from expected_policies) union all (select * from expected_policies except select * from actual_policies)) into drift;
  if drift then raise exception 'MP-4 rollback postcondition guard failed: policy manifest drift'; end if;
...
actual_functions as (
  ...
  select exists ((select function_name,identity_arguments,language_name,owner_name,volatility,security_definer,
      ...
      execute_grantees, normalized_body from actual_functions
      where function_name in (select function_name from expected_functions))) into drift;
  if drift then raise exception 'MP-4 rollback postcondition guard failed: function manifest drift'; end if;
end;
$guard$;
do $package_b_helper_guard$
begin
