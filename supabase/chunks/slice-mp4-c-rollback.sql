...
begin
 if current_user<>'postgres' or session_user<>'postgres' then raise exception 'Slice MP4-C ROLLBACK refused: owner SQL Editor context required' using errcode='42501'; end if;
...
manifest as (
 ...
 and (select count(*) from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role'))=5) and (m.policy_n=17 and m.policy_h='84def60822028f0e6206c1ab4e780cb3')),((m.relation_n=6 and m.relation_h='95b0d6bfa747399c5b7d9d412813ed53'
 and m.column_n=31 and m.column_h='f856c2e4647a77101952938c138cc904'
 and m.constraint_n=28 and m.constraint_h='99e20f84d2c518d4d36ea0bc27378de7'
 and m.index_n=23 and m.index_h='6e2c9273517aa80d33966f4c6d135b27'
 and m.trigger_n=0 and m.trigger_h='d41d8cd98f00b204e9800998ecf8427e'
 ...
 and m.membership_n=13 and m.membership_h='2c5626f6dd72493cd1f6b18bd20a6036'
 and not exists((select rolname,rolsuper,rolinherit,rolbypassrls from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role')) except (values ('anon'::name,false,true,false),('audit_readonly'::name,false,false,false),('authenticated'::name,false,true,false),('postgres'::name,false,true,true),('service_role'::name,false,true,true)))
 and (select count(*) from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role'))=5) and (m.policy_n=17 and m.policy_h='88b11cfcc7f9dc11e40259cfc5c67dd3')) into source_ok,target_ok from manifest m;
 if target_ok then return; end if;
 if not source_ok then raise exception 'Slice MP4-C ROLLBACK refused: exact source/target manifest not found' using errcode='55000'; end if;
 alter policy "Unbanned users manage own favorites" on public.favorites using (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id)) with check (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id));
alter policy "Users can read their own favorites" on public.favorites using (public.current_user_owns_profile(profile_id));
alter policy "Unbanned users follow from own profile" on public.follows with check (not public.current_user_is_banned() and public.current_user_owns_profile(follower_profile_id));
alter policy "Unbanned users unfollow from own profile" on public.follows using (not public.current_user_is_banned() and public.current_user_owns_profile(follower_profile_id));
alter policy "Unbanned users add own RSVP" on public.vendor_events with check (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id));
alter policy "Unbanned users remove own RSVP" on public.vendor_events using (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id));
alter policy "Unbanned users create own notice posts" on public.notice_posts with check (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id));
alter policy "Unbanned users delete own notice posts" on public.notice_posts using (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id));
alter policy "Unbanned users add own reactions" on public.notice_reactions with check (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id));
alter policy "Unbanned users remove own reactions" on public.notice_reactions using (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id));
alter policy "Unbanned users subscribe to event notifications" on public.event_notifications with check (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id));
alter policy "Unbanned users unsubscribe from event notifications" on public.event_notifications using (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id));
alter policy "Users can manage their own event notifications" on public.event_notifications using (public.current_user_owns_profile(profile_id));
end
$transition$;
do $post$
declare ok boolean;
...
manifest as (
 ...
 and m.membership_n=13 and m.membership_h='2c5626f6dd72493cd1f6b18bd20a6036'
 and not exists((select rolname,rolsuper,rolinherit,rolbypassrls from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role')) except (values ('anon'::name,false,true,false),('audit_readonly'::name,false,false,false),('authenticated'::name,false,true,false),('postgres'::name,false,true,true),('service_role'::name,false,true,true)))
 and (select count(*) from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role'))=5) and (m.policy_n=17 and m.policy_h='88b11cfcc7f9dc11e40259cfc5c67dd3')) into ok from manifest m;
 if not ok then raise exception 'Slice MP4-C ROLLBACK postcondition failed' using errcode='55000'; end if;
end
$post$;
commit;
