...
-- Restores the three replaced trigger functions to their exact owner-reviewed
-- pre-MP4-E bodies/settings. Run slice-mp4-e-rollback-verify.sql afterward.
begin;
...
begin
 ...
 and to_regprocedure('public.guard_conversation_unread_participants()') is null and to_regprocedure('public.guard_conversation_participant_state()') is null
...
begin
 ...
 if to_regprocedure('public.guard_conversation_participants()') is not null or to_regprocedure('public.guard_message_active_participant()') is not null or to_regprocedure('public.guard_conversation_unread_participants()') is not null or to_regprocedure('public.guard_conversation_participant_state()') is not null then raise exception 'MP4-E rollback postcondition: target guards remain'; end if;
 if not exists(select 1 from pg_constraint where conrelid='public.conversations'::regclass and conname='conversations_buyer_seller_differ' and lower(regexp_replace(pg_get_constraintdef(oid,false),'[[:space:]]+',' ','g'))='check ((buyer_profile_id <> seller_profile_id))') then raise exception 'MP4-E rollback postcondition: source differ constraint mismatch'; end if;
