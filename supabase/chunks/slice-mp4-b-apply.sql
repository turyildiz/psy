...
lock table public.profiles,public.listings in access exclusive mode;

do $transition$
declare source_ok boolean; target_ok boolean;
begin
 if current_user<>'postgres' or session_user<>'postgres' then raise exception 'Slice MP4-B APPLY refused: owner SQL Editor context required' using errcode='42501'; end if;
 with
...
manifest as (
 ...
 select (m.relation_n=2 and m.relation_h='f1fe7c4aeb160fea5044ab172d5a863d'
 ...
 and not exists(select 1 from pg_attribute a where a.attrelid=to_regclass('public.profiles') and a.attnum>0 and not a.attisdropped and has_column_privilege('authenticated','public.profiles',a.attname,'INSERT'))) into source_ok,target_ok from manifest m;
 if target_ok then return; end if;
 if not source_ok then raise exception 'Slice MP4-B APPLY refused: exact source/target manifest not found' using errcode='55000'; end if;

  execute 'drop policy "Unbanned users insert own profiles" on public.profiles';
  execute $sql$alter policy "Unbanned users update own profiles" on public.profiles
    using (id=(select public.current_active_profile_id()) and not (select public.current_user_is_banned()))
    with check (id=(select public.current_active_profile_id()) and not (select public.current_user_is_banned()))$sql$;
  execute $sql$alter policy "Owners can read own private listings" on public.listings
    using (profile_id=(select public.current_active_profile_id()))$sql$;
  execute $sql$alter policy "Unbanned owners create active listings" on public.listings
    with check (
      not (select public.current_user_is_banned())
      and status='active'::public.listing_status
      and admin_unpublished_at is null
      and admin_unpublished_by is null
      and profile_id=(select public.current_active_profile_id())
      and (select public.current_user_is_active_unsuspended_profile(public.current_active_profile_id()))
    )$sql$;
  execute $sql$alter policy "Unbanned owners delete own draft listings" on public.listings
    using (not (select public.current_user_is_banned()) and status='draft'::public.listing_status and admin_unpublished_at is null and profile_id=(select public.current_active_profile_id()))$sql$;
  execute $sql$alter policy "Unbanned owners update own listings" on public.listings
    using (not (select public.current_user_is_banned()) and profile_id=(select public.current_active_profile_id()))
    with check (not (select public.current_user_is_banned()) and profile_id=(select public.current_active_profile_id()) and (admin_unpublished_at is null or status='draft'::public.listing_status))$sql$;
  execute 'revoke insert on table public.profiles from authenticated';

end
$transition$;

-- Complete transactional postcondition: every unchanged dependency plus exact after policy/ACL state.
do $post$
declare ok boolean;
begin
 with
...
manifest as (
 ...
 and m.profile_acl_h=(case when current_setting('server_version_num')::int>=170000 then '29e23694b3fc1e0f9e48ba0a00c7925d' else 'eed8b709d0bf2766aa213cd75afc8e27' end)
 and not has_table_privilege('authenticated','public.profiles','INSERT')
 and not exists(select 1 from pg_attribute a where a.attrelid=to_regclass('public.profiles') and a.attnum>0 and not a.attisdropped and has_column_privilege('authenticated','public.profiles',a.attname,'INSERT'))) into ok from manifest m;
 if not ok then raise exception 'Slice MP4-B APPLY postcondition failed' using errcode='55000'; end if;
end
$post$;
commit;
