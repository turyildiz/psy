...
begin
 if current_user<>'postgres' or session_user<>'postgres' then raise exception 'Slice MP4-B ROLLBACK refused: owner SQL Editor context required' using errcode='42501'; end if;
 with
...
manifest as (
 ...
 and not exists(select 1 from pg_attribute a where a.attrelid=to_regclass('public.profiles') and a.attnum>0 and not a.attisdropped and has_column_privilege('authenticated','public.profiles',a.attname,'INSERT'))),(m.relation_n=2 and m.relation_h='f1fe7c4aeb160fea5044ab172d5a863d'
 and m.listing_acl_n=(case when current_setting('server_version_num')::int>=170000 then 33 else 29 end)
 and m.listing_acl_h=(case when current_setting('server_version_num')::int>=170000 then '9999dff08df8d6692a1f77fa6475dd58' else '6ec0e2d64c5630ec271f4ca9dc26c7b0' end)
 and m.profile_colacl_n=24 and m.profile_colacl_h='c30def816773ab9c1a81dbeecfc1f53d' and m.listing_colacl_n=0
 and m.default_acl_n=(case when current_setting('server_version_num')::int>=170000 then 312 else 286 end)
 ...
 and has_table_privilege('authenticated','public.profiles','INSERT')) into source_ok,target_ok from manifest m;
 if target_ok then return; end if;
 if not source_ok then raise exception 'Slice MP4-B ROLLBACK refused: exact source/target manifest not found' using errcode='55000'; end if;

  execute $sql$create policy "Unbanned users insert own profiles" on public.profiles for insert to authenticated
    with check (auth.uid()=user_id and not public.current_user_is_banned())$sql$;
  execute $sql$alter policy "Unbanned users update own profiles" on public.profiles
    ...
    with check (auth.uid()=user_id and not public.current_user_is_banned())$sql$;
  execute $sql$alter policy "Owners can read own private listings" on public.listings using (public.current_user_owns_profile(profile_id))$sql$;
  execute $sql$alter policy "Unbanned owners create active listings" on public.listings
    with check (not public.current_user_is_banned() and status='active'::public.listing_status and admin_unpublished_at is null and admin_unpublished_by is null and public.current_user_owns_unsuspended_profile(profile_id))$sql$;
  execute $sql$alter policy "Unbanned owners delete own draft listings" on public.listings
    using (not public.current_user_is_banned() and status='draft'::public.listing_status and admin_unpublished_at is null and public.current_user_owns_profile(profile_id))$sql$;
  execute $sql$alter policy "Unbanned owners update own listings" on public.listings
    using (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id))
    with check (not public.current_user_is_banned() and public.current_user_owns_profile(profile_id) and (admin_unpublished_at is null or status='draft'::public.listing_status))$sql$;
  execute 'grant insert on table public.profiles to authenticated';

end
$transition$;

-- Complete transactional postcondition: every unchanged dependency plus exact restored policy/ACL state.
do $post$
declare ok boolean;
begin
 with
...
manifest as (
 ...
 and m.profile_acl_n=(case when current_setting('server_version_num')::int>=170000 then 31 else 27 end)
 and m.profile_acl_h=(case when current_setting('server_version_num')::int>=170000 then 'c8b27bbddc599663c22923dcfbd164ab' else '438c33680ce845f8483caaf812697b25' end)
 and has_table_privilege('authenticated','public.profiles','INSERT')) into ok from manifest m;
 if not ok then raise exception 'Slice MP4-B ROLLBACK postcondition failed' using errcode='55000'; end if;
end
$post$;
commit;
