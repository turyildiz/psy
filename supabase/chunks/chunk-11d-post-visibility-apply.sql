...
-- APPLY: public versus members-only post and reaction visibility.
--
-- Status: APPLIED AND VERIFIED (owner-confirmed 2026-08-07).
-- Dependency: Chunk 11D preflight returned GO against owner-applied and verified
-- Chunks 11A, 11B, and 11C.
--
...
-- Fail closed unless the exact captured pre-11D policy, helper, ACL, RLS, and
-- role state remains installed. Source bodies are compared directly after
-- whitespace normalization; no unobserved catalog-body hashes are used.
do $preconditions$
declare
...
begin
  if current_user <> 'postgres' then
    raise exception 'Chunk 11D apply must run as postgres';
  end if;

  if current_database() <> 'postgres' then
    raise exception 'Chunk 11D precondition failed: unexpected database name';
  end if;
  ...
  if not coalesce((
    ...
    raise exception 'Chunk 11D precondition failed: postgres role attributes drifted';
  end if;
  ...
  if not coalesce((
    ...
    raise exception 'Chunk 11D precondition failed: service_role BYPASSRLS drifted';
  end if;

  if to_regclass('public.posts') is null
     or to_regclass('public.post_reactions') is null
     or author_helper_oid is null
     or reaction_helper_oid is null then
    raise exception 'Chunk 11D precondition failed: Chunk 11 dependency missing';
  end if;
  ...
  if exists (
    ...
    raise exception 'Chunk 11D precondition failed: conflicting visibility helper name exists';
  end if;
  ...
  if (
    ...
    raise exception 'Chunk 11D precondition failed: baseline helper overload set drifted';
  end if;
  ...
  if (
    ...
    raise exception 'Chunk 11D precondition failed: table owner/RLS state drifted';
  end if;
  ...
  if not coalesce((
    ...
    raise exception 'Chunk 11D precondition failed: show_in_stream column drifted';
  end if;
  ...
  if (
    ...
    raise exception 'Chunk 11D precondition failed: posts policy set drifted';
  end if;
  ...
  if (
    ...
    raise exception 'Chunk 11D precondition failed: reaction policy set drifted';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Chunk 11D precondition failed: author helper drifted';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Chunk 11D precondition failed: reaction helper drifted';
  end if;
  ...
  if exists (
    ...
    raise exception 'Chunk 11D precondition failed: table ACL set drifted';
  end if;
  ...
  if exists (
    ...
    raise exception 'Chunk 11D precondition failed: function ACL set drifted';
  end if;
  ...
  if exists (
    ...
    raise exception 'Chunk 11D precondition failed: mutation RPC signature/ACL drifted';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Chunk 11D precondition failed: Hero invariant drifted';
  end if;
end;
$preconditions$;

create function public.current_user_can_read_post(
  target_profile_id uuid,
...
as $function$
  select
    public.current_user_can_read_post_author($1)
    and ($2 or auth.role() = 'authenticated');
$function$;

alter function public.current_user_can_read_post(uuid, boolean)
  owner to postgres;

revoke all privileges on function public.current_user_can_read_post(uuid, boolean)
  from public, anon, authenticated, service_role;
grant execute on function public.current_user_can_read_post(uuid, boolean)
  to anon, authenticated;

comment on function public.current_user_can_read_post(uuid, boolean) is
  'Central post read decision: public when Stream-enabled, otherwise authenticated members only, with existing author-ban/admin behavior.';

-- Replace the only posts SELECT policy. The old broad qualifier must not survive
-- as another permissive branch because permissive policies combine with OR.
drop policy "Visible posts are publicly readable" on public.posts;
create policy "Visible posts are publicly readable"
on public.posts
for select
to public
using (public.current_user_can_read_post(profile_id, show_in_stream));

-- Preserve the existing reactor-ban predicate and delegate parent visibility to
-- the same centralized post helper used by public.posts RLS.
...
as $function$
  select
    ...
    and coalesce((
      select public.current_user_can_read_post(
        post.profile_id,
        post.show_in_stream
      )
...
-- CREATE OR REPLACE retains ACLs. Reassert the exact reviewed set.
...
begin
  if visibility_helper_oid is null or reaction_helper_oid is null then
    raise exception 'Chunk 11D postcondition failed: helper missing';
  end if;
  ...
  if (
    ...
    raise exception 'Chunk 11D postcondition failed: conflicting visibility helper overload';
  end if;
  ...
  if (
    ...
    raise exception 'Chunk 11D postcondition failed: baseline helper overload set drifted';
  end if;
  ...
  if (
    ...
    raise exception 'Chunk 11D postcondition failed: table owner/RLS mismatch';
  end if;
  ...
  if (
    ...
    raise exception 'Chunk 11D postcondition failed: posts policy mismatch';
  end if;
  ...
  if (
    ...
    raise exception 'Chunk 11D postcondition failed: reaction policy mismatch';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Chunk 11D postcondition failed: visibility helper mismatch';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Chunk 11D postcondition failed: reaction helper mismatch';
  end if;
  ...
  if exists (
    with actual(function_name, grantee_name, privilege_type, is_grantable, grantor_name) as (
      ...
      from pg_proc p
      cross join lateral aclexplode(p.proacl) as acl
      where p.oid in (visibility_helper_oid, reaction_helper_oid)
        and acl.grantee <> p.proowner
    ),
    expected(function_name, grantee_name, privilege_type, is_grantable, grantor_name) as (
      values
    ...
    raise exception 'Chunk 11D postcondition failed: function ACL set mismatch';
  end if;
  ...
  if exists (
    ...
    raise exception 'Chunk 11D postcondition failed: table ACL set mismatch';
  end if;
  ...
  if exists (
    ...
    raise exception 'Chunk 11D postcondition failed: mutation RPC signature/ACL mismatch';
  end if;
  ...
  if not coalesce((
    ...
    raise exception 'Chunk 11D postcondition failed: service_role BYPASSRLS mismatch';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Chunk 11D postcondition failed: Hero invariant mismatch';
  end if;
end;
$postconditions$;
