...
-- OWNER-RUN READ-ONLY PREFLIGHT — NOT EXECUTED BY THE REPOSITORY AGENT.
--
-- One final row verifies the complete approved Package A pre-cutover baseline
-- and fails closed if any same-name Package B function or overload already
-- exists. This file performs no DDL, DML, application/RPC function call, or
-- session mutation.
...
package_b_absence as (
  select count(*)::bigint as helper_name_or_overload_count
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
  where n.nspname = 'public'
    and p.proname in (
...
package_b_fixtures as (
  select
    count(distinct p.user_id) filter (
      where u.banned_at is null
        and u.role::text not in ('admin', 'super_admin')
    )::bigint as ordinary_owner_count,
    count(distinct p.user_id)::bigint as distinct_profile_owner_count,
    count(distinct u.id) filter (
      where u.banned_at is null
        and u.role::text in ('admin', 'super_admin')
    )::bigint as active_admin_count,
    count(distinct p.user_id) filter (
      where u.banned_at is null
        and u.role::text in ('admin', 'super_admin')
    )::bigint as active_admin_profile_owner_count
  from public.users u
  left join public.profiles p on p.user_id = u.id
)
select
  case
    when fs.overall_status = 'STOP'
      or b.helper_name_or_overload_count <> 0 then 'STOP'
    when fs.overall_status = 'UNPROVEN'
      or f.ordinary_owner_count = 0
      or f.distinct_profile_owner_count < 2
      or f.active_admin_count = 0
      or f.active_admin_profile_owner_count = 0 then 'UNPROVEN'
    else 'GO'
  end as overall_status,
  fs.overall_status as package_a_baseline_status,
  case when b.helper_name_or_overload_count = 0 then 'GO'
       else 'STOP' end as package_b_objects_absent_status,
  case when f.ordinary_owner_count > 0
      and f.distinct_profile_owner_count >= 2
      and f.active_admin_count > 0
      and f.active_admin_profile_owner_count > 0 then 'GO'
    else 'UNPROVEN' end as package_b_fixture_status,
  b.helper_name_or_overload_count,
  array_cat(
    fs.stop_findings,
    case when b.helper_name_or_overload_count <> 0
      then array['package_b_helper_name_or_overload_already_exists']::text[]
      else array[]::text[] end
  ) as stop_findings,
  array_cat(
    ...
    array_remove(array[
      case when f.ordinary_owner_count = 0
        then 'package_b_ordinary_owner_fixture' end,
      case when f.distinct_profile_owner_count < 2
        then 'package_b_unrelated_profile_fixture' end,
      case when f.active_admin_count = 0
        then 'package_b_active_admin_fixture' end,
      case when f.active_admin_profile_owner_count = 0
        then 'package_b_admin_profile_fixture' end
    ], null)
  ) as unproven_findings,
  array_cat(
    fs.unavailable_fixtures,
    array_remove(array[
      case when f.ordinary_owner_count = 0 then 'ordinary_owner' end,
      case when f.distinct_profile_owner_count < 2 then 'unrelated_profile' end,
      case when f.active_admin_count = 0 then 'active_admin' end,
      case when f.active_admin_profile_owner_count = 0
        then 'admin_profile' end
    ], null)
  ) as unavailable_fixtures,
  ...
  '83d35c728031ce2836246d9677aead37ad93fde0'::text
    as package_a_app_source_revision
from final_status fs
cross join package_b_absence b
cross join package_b_fixtures f
