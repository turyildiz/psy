...
HOST=${MP4F_HOST:-/tmp/mp4f-sock}; PORT=${MP4F_PORT:-55443}; USER=${MP4F_USER:-postgres}; ADMIN=${MP4F_ADMIN_USER:-supabase_admin}
ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd); DB=${MP4F_DB:-mp4f_disposable_$$}; LOG_DIR=${MP4F_LOG_DIR:-$(mktemp -d)}; mkdir -p "$LOG_DIR"
P=(psql -X -q -v ON_ERROR_STOP=1 -h "$HOST" -p "$PORT" -U "$USER" -d "$DB")
FILES=("$ROOT/supabase/chunks/slice-mp4-f-preflight.sql" "$ROOT/supabase/chunks/slice-mp4-f-apply.sql" "$ROOT/supabase/chunks/slice-mp4-f-verify.sql" "$ROOT/supabase/chunks/slice-mp4-f-rollback.sql" "$ROOT/supabase/chunks/slice-mp4-f-rollback-verify.sql" "$ROOT/supabase/chunks/slice-mp4-f-disposable-runtime.sql" "$ROOT/supabase/chunks/slice-mp4-f-disposable-harness.sh")
...
if ((version>=170000)); then echo POSTGRESQL_17_SEMANTICS=PASS; elif [[ ${MP4F_ALLOW_SUPPORTING_PG16:-0} == 1 ]]; then echo "POSTGRESQL_17_SEMANTICS=UNAVAILABLE(server_version_num=$version); explicit PG16 supporting run"; else echo "POSTGRESQL_17_SEMANTICS=BLOCKED(server_version_num=$version)" >&2; exit 1; fi
...
state(){ "${P[@]}" -At -c "with x as(select c.relname||'|'||p.polname||'|'||p.polcmd::text||'|'||coalesce(pg_get_expr(p.polqual,p.polrelid,false),'')||'|'||coalesce(pg_get_expr(p.polwithcheck,p.polrelid,false),'') v from pg_policy p join pg_class c on c.oid=p.polrelid where p.polrelid in('public.conversations'::regclass,'public.messages'::regclass,'public.conversation_participant_state'::regclass))select md5(string_agg(v,E'\n' order by v collate \"C\")) from x;"; }
verdict "$ROOT/supabase/chunks/slice-mp4-f-preflight.sql" preflight-default; verdict "$ROOT/supabase/chunks/slice-mp4-f-preflight.sql" preflight-hostile yes
if "${P[@]}" -f "$ROOT/supabase/chunks/slice-mp4-f-disposable-runtime.sql" >"$LOG_DIR/red-before-apply.log" 2>&1; then echo 'RED failed: target behavior unexpectedly passed before MP4-F' >&2; exit 1; fi
echo RED_POLICY_MATRIX=PASS_EXPECTED_FAILURE
"${P[@]}" -f "$ROOT/supabase/chunks/slice-mp4-f-apply.sql" >"$LOG_DIR/apply.log"; before=$(state); "${P[@]}" -f "$ROOT/supabase/chunks/slice-mp4-f-apply.sql" >"$LOG_DIR/apply-noop.log"; [[ $before == "$(state)" ]]; echo APPLY_NOOP=PASS
verdict "$ROOT/supabase/chunks/slice-mp4-f-verify.sql" verify-default; verdict "$ROOT/supabase/chunks/slice-mp4-f-verify.sql" verify-hostile yes
"${P[@]}" -f "$ROOT/supabase/chunks/slice-mp4-f-disposable-runtime.sql" >"$LOG_DIR/runtime.log" 2>&1; grep -q MP4F_POLICY_BEHAVIOR_MATRIX=PASS "$LOG_DIR/runtime.log"; grep -q REALTIME_JWT_POLICY_CONTEXT_DATABASE=PASS "$LOG_DIR/runtime.log"; grep -q REALTIME_WORKER_DELIVERY_OWNER_GATE=UNPROVEN "$LOG_DIR/runtime.log"; echo POLICY_BEHAVIOR_MATRIX=PASS; echo REALTIME_DATABASE_CONTEXT=PASS_OWNER_DELIVERY_GATE_UNPROVEN
# Planner/InitPlan evidence on 500 candidate conversations/messages/state rows.
"${P[@]}" <<'SQL' >"$LOG_DIR/planner-seed.log"
select set_config('request.jwt.claims','{"sub":"bb000000-0000-4000-8000-000000000001","role":"authenticated","session_id":"bb000000-0000-4000-8000-000000000011"}',false);
insert into public.conversations(id,buyer_profile_id,seller_profile_id)
 select md5('mp4f-plan-conv-'||g)::uuid,'b2000000-0000-4000-8000-000000000001','c3000000-0000-4000-8000-000000000001' from generate_series(1,500) g;
insert into public.messages(id,conversation_id,sender_profile_id,body)
 select md5('mp4f-plan-msg-'||g)::uuid,md5('mp4f-plan-conv-'||g)::uuid,'b2000000-0000-4000-8000-000000000001','planner row' from generate_series(1,500) g;
insert into public.conversation_participant_state(conversation_id,profile_id)
 select md5('mp4f-plan-conv-'||g)::uuid,'b2000000-0000-4000-8000-000000000001' from generate_series(1,500) g;
SQL
CLAIMS="set request.jwt.claims='{\"sub\":\"bb000000-0000-4000-8000-000000000001\",\"role\":\"authenticated\",\"session_id\":\"bb000000-0000-4000-8000-000000000011\"}';set role authenticated;"
"${P[@]}" -At -c "$CLAIMS explain(analyze,buffers,format json) select * from public.conversations" >"$LOG_DIR/plan-conversations.json"
"${P[@]}" -At -c "$CLAIMS explain(analyze,buffers,format json) select * from public.messages" >"$LOG_DIR/plan-messages.json"
"${P[@]}" -At -c "$CLAIMS explain(analyze,buffers,format json) select * from public.conversation_participant_state" >"$LOG_DIR/plan-state.json"
"${P[@]}" -At -c "$CLAIMS explain(format json) insert into public.conversations(buyer_profile_id,seller_profile_id) values('b2000000-0000-4000-8000-000000000001','c3000000-0000-4000-8000-000000000001')" >"$LOG_DIR/plan-conversation-insert.json"
"${P[@]}" -At -c "$CLAIMS explain(format json) insert into public.messages(conversation_id,sender_profile_id,body) values('e0000000-0000-4000-8000-000000000001','b2000000-0000-4000-8000-000000000001','plan')" >"$LOG_DIR/plan-message-insert.json"
python3 -c 'import json,sys,pathlib
for f in sys.argv[1:]:
 d=json.loads(pathlib.Path(f).read_text()); nodes=[]
 def walk(x):
  if isinstance(x,dict): nodes.append(x);[walk(v) for v in x.values()]
  elif isinstance(x,list): [walk(v) for v in x]
 walk(d)
 init=[n for n in nodes if n.get("Parent Relationship")=="InitPlan"]
 assert init,(f,"missing InitPlan")
 if "insert" not in f: assert any(n.get("Actual Loops")==1 for n in init),(f,"InitPlan loops")
 print(f+"=INITPLAN_PASS")' "$LOG_DIR"/plan-*.json >"$LOG_DIR/planner-assert.log"
