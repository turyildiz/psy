...
expected_private_table_acl(g,r,p,x) as (select 'postgres'::text,'postgres'::text,p,false from available),
actual_private_table_acl(g,r,p,x) as (select a.grantor::regrole::text,case when a.grantee=0 then 'PUBLIC' else a.grantee::regrole::text end,a.privilege_type::text,a.is_grantable from pg_class c cross join lateral aclexplode(coalesce(c.relacl,acldefault('r',c.relowner))) a where c.oid=to_regclass('private.account_session_active_profiles')),
expected_private_schema_acl(g,r,p,x) as (values ('postgres'::text,'postgres'::text,'CREATE'::text,false),('postgres','postgres','USAGE',false)),
actual_private_schema_acl(g,r,p,x) as (select a.grantor::regrole::text,case when a.grantee=0 then 'PUBLIC' else a.grantee::regrole::text end,a.privilege_type::text,a.is_grantable from pg_namespace n cross join lateral aclexplode(coalesce(n.nspacl,acldefault('n',n.nspowner))) a where n.nspname='private'),
fingerprints as (
 select
 ...
 (select md5(c.relowner::regrole::text||'|'||c.relkind::text||'|'||c.relpersistence::text||'|'||c.relrowsecurity||'|'||c.relforcerowsecurity||'|'||c.relreplident::text) from pg_class c where c.oid=to_regclass('private.account_session_active_profiles')) private_rel,
 (select md5(n.nspowner::regrole::text) from pg_namespace n where n.nspname='private') private_schema
 ...
 ('profiles_constraints_exact',(select profile_constraints='f4ac02be62984924b395f7b701a700f3' from fingerprints),null,'complete constraint manifest'),
 ...
 ('package_b_helpers_exact',(select helper_hash='d175baa081926c85004e36268f061f76' from fingerprints),null,'helper definitions'),
 ...
 ('signup_exact',(select signup_hash='59ab4dce2526438016742af297aadc55' from fingerprints),null,'signup definition'),
 ('mp3_functions_exact',(select mp3_function_hash='37642125b28b41fd83eb27d62e55e116' and mp3_acl_hash='278f9ec775ed45028d4d817ab12b0eb7' from fingerprints),null,'function definitions/ACL/overloads'),
 ('private_state_exact',(select private_constraints='2890dfa115b3b4d990522a2ba9c49701' and private_indexes='ae9b1f423455163225eaaa07886a845e' and private_rel='f9a66a264d09636f09eac24c19eb6b43' and private_schema='e8a48653851e28c69d0506508fb27fc5' from fingerprints) and not exists((select * from actual_private_table_acl except select * from expected_private_table_acl) union all (select * from expected_private_table_acl except select * from actual_private_table_acl)) and not exists((select * from actual_private_schema_acl except select * from expected_private_schema_acl) union all (select * from expected_private_schema_acl except select * from actual_private_schema_acl)),null,'private schema/table/ACL'),
