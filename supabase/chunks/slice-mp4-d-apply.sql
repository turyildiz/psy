...
begin
 if current_user<>'postgres' or session_user<>'postgres' then raise exception 'Slice MP4-D APPLY refused: owner SQL Editor context required' using errcode='42501'; end if;
 with
relation_state as (
...
manifest as (
 ...
 and m.wall_function_n=23 and m.wall_function_h='650916cc5006e33fd07e4fe90a568d81'),(m.relation_n=3 and m.relation_h='896fc89a04d20b4a954675e4b6617c7d'
 and m.column_n=18 and m.column_h='7e21ce93ef5410cf8b1deddef17f3dfc'
 and m.constraint_n=15 and m.constraint_h='b0d239ef13d46d549eb86256aafb2635'
 and m.index_n=8 and m.index_h='de0159494a3da73e317dc15ee2d38a4e'
 and m.trigger_n=2 and m.trigger_h='b0e55d8bf75f44aed109a3f69b0d2a4e'
 ...
 and not exists((select rolname,rolsuper,rolinherit,rolbypassrls from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role')) except (values ('anon'::name,false,true,false),('audit_readonly'::name,false,false,false),('authenticated'::name,false,true,false),('postgres'::name,false,true,true),('service_role'::name,false,true,true)))
 and (select count(*) from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role'))=5
 and m.wall_function_n=23 and m.wall_function_h='9ee1c2705f46e6c97892fc241344ebee') into source_ok,target_ok from manifest m;
 if target_ok then return; end if;
 if not source_ok then raise exception 'Slice MP4-D APPLY refused: exact source/target manifest not found' using errcode='55000'; end if;
 execute $ddl_0$
...
begin
  ...
  if target_profile_id is distinct from (select public.current_active_profile_id()) then
...
begin
  ...
  where post.id = target_post_id
    and post.profile_id = (select public.current_active_profile_id());
...
begin
  ...
  where reaction.post_id = target_post_id
    ...
    and reaction.profile_id = (select public.current_active_profile_id());
...
begin
  ...
  if author_profile_id is distinct from (select public.current_active_profile_id()) then
...
manifest as (
 ...
 and not exists((select rolname,rolsuper,rolinherit,rolbypassrls from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role')) except (values ('anon'::name,false,true,false),('audit_readonly'::name,false,false,false),('authenticated'::name,false,true,false),('postgres'::name,false,true,true),('service_role'::name,false,true,true)))
 and (select count(*) from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role'))=5
 and m.wall_function_n=23 and m.wall_function_h='9ee1c2705f46e6c97892fc241344ebee') into ok from manifest m;
 if not ok then raise exception 'Slice MP4-D APPLY postcondition failed' using errcode='55000'; end if;
end
$post$;
commit;
