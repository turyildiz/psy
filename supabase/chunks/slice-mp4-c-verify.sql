...
checks(name,ok,detail) as (
 ...
 ('dependency_manifest_exact',(m.relation_n=6 and m.relation_h='95b0d6bfa747399c5b7d9d412813ed53'
 ...
 and (select count(*) from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role'))=5),'six-table anatomy/ACL/publication/helper/role drift'),
 ('verify_policy_exact',(m.policy_n=17 and m.policy_h='84def60822028f0e6206c1ab4e780cb3'),'policy manifest drift'),
 ('active_policy_shape_exact',(select count(*) from pg_policy p join pg_class c on c.oid=p.polrelid where c.relnamespace='public'::regnamespace and c.relname in ('event_notifications','favorites','follows','notice_posts','notice_reactions','vendor_events') and ((pg_get_expr(p.polqual,p.polrelid) like '%current_active_profile_id%' and p.polqual is not null) or (pg_get_expr(p.polwithcheck,p.polrelid) like '%current_active_profile_id%' and p.polwithcheck is not null)))=13,'converted active policy count drift'),
 ('self_follow_and_public_reads_preserved',exists(select 1 from pg_constraint where conrelid=to_regclass('public.follows') and conname='follows_check') and (select count(*) from pg_policy p join pg_class c on c.oid=p.polrelid where c.relnamespace='public'::regnamespace and ((c.relname='follows' and p.polname='follows_select') or (c.relname='vendor_events' and p.polname='Vendor events are publicly readable') or (c.relname='notice_posts' and p.polname='Notice posts are publicly readable') or (c.relname='notice_reactions' and p.polname='Reactions are publicly readable')))=4,'self-follow/public policy drift')
 ) v(name,ok,detail)
...
select 'SLICE_MP4_C_VERIFY' package,case when all_ok then 'GO' else 'STOP' end verdict,passed,total,findings,'Exact six-table catalog pins come from the 2026-08-15 MP4-C live-evidence bundle; active-authority helper pins come from applied MP4-A after-state. No row aggregates were admitted. Account notification preference is outside these six tables and is untouched, not independently repinned here.'::text boundary from summary;
rollback;
