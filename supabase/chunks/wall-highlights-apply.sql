-- psy.market Wall highlights: posts highlight state, max-five fence, active-profile toggle RPC
-- Policy fingerprints use plain two-argument pg_get_expr(), matching the admitted MP4-D evidence.
-- Exact source pins: live-verified MP4-D after-state (2026-08-15). No live DDL/DML was run by the author.
...
begin
 if current_user<>'postgres' or session_user<>'postgres' then raise exception 'Wall highlights APPLY refused: owner SQL Editor context required' using errcode='42501'; end if;
 with
relation_state as (
...
column_state as (
 ...
 from pg_class c join pg_namespace n on n.oid=c.relnamespace join pg_attribute a on a.attrelid=c.oid left join pg_attrdef d on d.adrelid=a.attrelid and d.adnum=a.attnum where n.nspname='public' and c.relname in ('post_reactions','post_write_rate_limits','posts') and a.attnum>0 and not a.attisdropped and not (c.relname='posts' and a.attname in ('is_highlighted','highlighted_at'))
),
highlight_column_state as (
 select count(*)::int n,md5(coalesce(string_agg(c.relname||'|'||a.attname||'|'||regexp_replace(format_type(a.atttypid,a.atttypmod),'public\.','','g')||'|'||case when a.attnotnull then 't' else 'f' end||'|'||regexp_replace(coalesce(pg_get_expr(d.adbin,d.adrelid,true),''),'public\.','','g')||'|'||case when a.attcollation=0 then '-' else regexp_replace(a.attcollation::regcollation::text,'public\.','','g') end||'|'||coalesce(nullif(a.attidentity::text,''),'<none>')||'|'||coalesce(nullif(a.attgenerated::text,''),'<none>')||'|'||a.attstorage::text,E'\n' order by c.relname collate "C",a.attname collate "C"),'')) h
 from pg_class c join pg_namespace n on n.oid=c.relnamespace join pg_attribute a on a.attrelid=c.oid left join pg_attrdef d on d.adrelid=a.attrelid and d.adnum=a.attnum where n.nspname='public' and c.relname='posts' and a.attname in ('is_highlighted','highlighted_at') and a.attnum>0 and not a.attisdropped
),
constraint_state as (
...
manifest as (
 select r.n relation_n,r.h relation_h,c.n column_n,c.h column_h,hc.n highlight_column_n,hc.h highlight_column_h,co.n constraint_n,co.h constraint_h,i.n index_n,i.h index_h,t.n trigger_n,t.h trigger_h,p.n policy_n,p.h policy_h,a.n acl_n,a.h acl_h,ca.n colacl_n,pu.n publication_n,pu.h publication_h,wf.n wall_function_n,wf.h wall_function_h,wa.n wall_acl_n,wa.h wall_acl_h,mf.n mp4a_function_n,mf.h mp4a_function_h,ma.n mp4a_acl_n,ma.h mp4a_acl_h,lf.n legacy_function_n,lf.h legacy_function_h,la.n legacy_acl_n,la.h legacy_acl_h from relation_state r cross join column_state c cross join highlight_column_state hc cross join constraint_state co cross join index_state i cross join trigger_state t cross join policy_state p cross join acl_state a cross join colacl_state ca cross join publication_state pu cross join wall_function_state wf cross join wall_acl_state wa cross join mp4a_function_state mf cross join mp4a_acl_state ma cross join legacy_function_state lf cross join legacy_acl_state la
)

 select ((m.relation_n=3 and m.relation_h='896fc89a04d20b4a954675e4b6617c7d'
 and m.column_n=18 and m.column_h='7e21ce93ef5410cf8b1deddef17f3dfc' and m.highlight_column_n=0 and m.highlight_column_h='d41d8cd98f00b204e9800998ecf8427e'
 ...
 and m.wall_function_n=23 and m.wall_function_h='9ee1c2705f46e6c97892fc241344ebee'
 ...
 and m.legacy_acl_n=8 and m.legacy_acl_h='6736d5d9cb627a2c79a7d2a84fb8f7d2') and (not exists((select rolname,rolsuper,rolinherit,rolbypassrls from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role')) except (values ('anon'::name,false,true,false),('audit_readonly'::name,false,false,false),('authenticated'::name,false,true,false),('postgres'::name,false,true,true),('service_role'::name,false,true,true))) and (select count(*) from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role'))=5)),((m.relation_n=3 and m.relation_h='896fc89a04d20b4a954675e4b6617c7d'
 and m.column_n=18 and m.column_h='7e21ce93ef5410cf8b1deddef17f3dfc' and m.highlight_column_n=2 and m.highlight_column_h='648cd29e2b2093884c479ee3a8680612'
 and m.constraint_n=16 and m.constraint_h='a75bda2d315fc51635e28c29d52cb5e6'
 and m.index_n=9 and m.index_h='64893d315dbb437172f40bcb1a1597be'
 and m.trigger_n=3 and m.trigger_h='b211f34861373d0f1ef7dbf072668e9b'
 and m.policy_n=2 and m.policy_h='4ed569e4742a87d96141c344ced1f5d8'
 and m.acl_n=(case when current_setting('server_version_num')::int>=170000 then 30 else 27 end)
 and m.acl_h=(case when current_setting('server_version_num')::int>=170000 then 'cc7fa3fcddd72c7542f85a0fb8fd10ef' else '51d2feec17ddf0a309e74ef4bc6e471b' end)
 and m.colacl_n=0 and m.publication_n=0 and m.publication_h='d41d8cd98f00b204e9800998ecf8427e'
 ...
 and m.mp4a_acl_n=7 and m.mp4a_acl_h='1a9bc3c64d199fbfe7463dc80a3f7fb1'
 and m.legacy_function_n=4 and m.legacy_function_h='587f39a8031bb763f494e3612df94b73'
 and m.legacy_acl_n=8 and m.legacy_acl_h='6736d5d9cb627a2c79a7d2a84fb8f7d2') and (not exists((select rolname,rolsuper,rolinherit,rolbypassrls from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role')) except (values ('anon'::name,false,true,false),('audit_readonly'::name,false,false,false),('authenticated'::name,false,true,false),('postgres'::name,false,true,true),('service_role'::name,false,true,true))) and (select count(*) from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role'))=5)) into source_ok,target_ok from manifest m;
 if target_ok then return; end if;
 if not source_ok then raise exception 'Wall highlights APPLY refused: exact source/target manifest not found' using errcode='55000'; end if;
 execute $ddl$alter table public.posts add column is_highlighted boolean not null default false, add column highlighted_at timestamp with time zone, add constraint posts_highlight_state_check check ((is_highlighted and highlighted_at is not null) or (not is_highlighted and highlighted_at is null))$ddl$;
 execute $ddl$create index posts_profile_highlighted_id_idx on public.posts (profile_id, highlighted_at desc, id desc) where is_highlighted$ddl$;
 execute $ddl$CREATE FUNCTION public.enforce_post_highlight_limit()
RETURNS trigger
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
begin
  if tg_when <> 'BEFORE'
    ...
    raise exception 'Unexpected enforce_post_highlight_limit trigger context'
      using errcode = '42501';
  end if;
  ...
  if tg_op = 'UPDATE'
     and new.is_highlighted is not distinct from old.is_highlighted then
    if new.highlighted_at is distinct from old.highlighted_at then
      raise exception 'Post highlight timestamp is trigger-owned'
        using errcode = '42501';
    end if;
    return new;
  end if;

  if not new.is_highlighted then
  ...
  where post.profile_id = new.profile_id
    and post.is_highlighted
    and (tg_op = 'INSERT' or post.id <> old.id);
...
$function$$ddl$;
 execute 'alter function public.enforce_post_highlight_limit() owner to postgres';
 execute 'revoke all on function public.enforce_post_highlight_limit() from public, anon, authenticated, service_role';
 execute 'grant execute on function public.enforce_post_highlight_limit() to postgres';
 execute $ddl$create trigger enforce_post_highlight_limit_trigger before insert or update of profile_id, is_highlighted, highlighted_at on public.posts for each row execute function public.enforce_post_highlight_limit()$ddl$;
 execute $ddl$CREATE FUNCTION public.toggle_post_highlight(target_post_id uuid, should_highlight boolean)
RETURNS void
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
begin
  ...
  if public.current_user_is_banned() then
    raise exception 'Banned users cannot change post highlights'
      using errcode = '42501';
  end if;

  if should_highlight is null then
    raise exception 'Highlight state must be true or false'
      using errcode = '22023';
  end if;

  active_profile_id := (select public.current_active_profile_id());
...
$function$$ddl$;
 execute 'alter function public.toggle_post_highlight(uuid, boolean) owner to postgres';
 execute 'revoke all on function public.toggle_post_highlight(uuid, boolean) from public, anon, service_role';
 execute 'grant execute on function public.toggle_post_highlight(uuid, boolean) to authenticated';
 execute 'grant execute on function public.toggle_post_highlight(uuid, boolean) to postgres';
end
$transition$;
do $post$
declare ok boolean;
...
manifest as (
 ...
 and m.mp4a_acl_n=7 and m.mp4a_acl_h='1a9bc3c64d199fbfe7463dc80a3f7fb1'
 and m.legacy_function_n=4 and m.legacy_function_h='587f39a8031bb763f494e3612df94b73'
 and m.legacy_acl_n=8 and m.legacy_acl_h='6736d5d9cb627a2c79a7d2a84fb8f7d2') and (not exists((select rolname,rolsuper,rolinherit,rolbypassrls from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role')) except (values ('anon'::name,false,true,false),('audit_readonly'::name,false,false,false),('authenticated'::name,false,true,false),('postgres'::name,false,true,true),('service_role'::name,false,true,true))) and (select count(*) from pg_roles where rolname in ('anon','audit_readonly','authenticated','postgres','service_role'))=5)) into ok from manifest m;
 if not ok then raise exception 'Wall highlights APPLY postcondition failed' using errcode='55000'; end if;
end
$post$;
commit;
