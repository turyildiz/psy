...
- MP-4 (policy/function conversion package): authored, validated, pushed — **NEVER applied to the live database.** Open before apply: deny-path verification needs a banned account; no admin ban UI exists, so a SQL fixture (temp ban + unban scripts for a demo account) was proposed but NOT completed. MP-4 apply is an open decision.
- Package D (privacy cutover: revoke broad SELECT, grant 12 safe columns): not authored, not applied.
