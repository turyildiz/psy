...
10. RECOVERY RESPONSE LOSS: V1 accepts the rare non-atomic handoff where Supabase may consume a one-time recovery OTP but the browser does not receive the encrypted recovery cookie because the HTTP response is lost. The safe recovery is to request a new reset link. Do not add durable database-backed recovery state before launch without a separate reviewed design and owner-applied migration approval.
