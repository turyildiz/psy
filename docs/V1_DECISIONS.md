...
- **Stream visibility:** Each post has a **show in Stream** checkbox, enabled by default. If unchecked, the post is hidden only from the Stream page. It remains visible on the author's Wall and in the Following feeds of users who follow that author. A post the author has excluded from Stream cannot be flagged for the Homepage Hero by admins.
