...
function FeaturedCard({ item }: { item: Listing }) {
  ...
  return (
    <Link href={`/listing/${item.id}`} className="apparel-featured-link" style={{ textDecoration: "none" }}>
      <div
        className="apparel-featured-card"
      ...
      >
        <div className="apparel-featured-image">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img className="apparel-featured-image-element" src={item.images[0] || "/listing-placeholder.webp"} onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = "/listing-placeholder.webp"; }} alt={item.title} style={{ transition: "transform 0.55s ease", transform: hov ? "scale(1.04)" : "scale(1)" }} />
        ...
        <div className="apparel-featured-content" style={{ padding: "16px 18px 20px" }}>
          <p className="apparel-featured-title" style={{ fontSize: "14px", fontWeight: 600, color: "var(--text)", marginBottom: "5px", lineHeight: 1.3 }}>{item.title}</p>
          {item.description && (
            <p className="apparel-featured-description" style={{ fontSize: "12px", color: "var(--text-light)", marginBottom: "12px", lineHeight: 1.5, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2 } as CSSProperties}>
              {item.description}
            </p>
          )}
          <div className="apparel-featured-price-row">
            <p className="apparel-featured-price" style={{ fontFamily: "'Bricolage Grotesque', var(--font-bricolage)", fontSize: "22px", fontWeight: 700, color: "var(--rust)", margin: 0 }}>
              {formatPrice(item.priceCents)}
            </p>
            <span className="apparel-featured-condition" style={{ fontSize: "10px", padding: "2px 8px", borderRadius: "4px", background: `${condColor}18`, color: condColor, fontWeight: 600 }}>
              {conditionLabels[item.condition]}
            </span>
          </div>
          {item.sellerHandle && (
            <div className="apparel-featured-seller-row" style={{ marginTop: "10px", paddingTop: "10px", borderTop: "1px solid var(--sand)" }}>
              <ProfileAvatar name={item.sellerName || item.sellerHandle} url={item.sellerAvatar} size={20} />
              <span className="apparel-featured-seller-handle" style={{ fontSize: "11px", color: "var(--text-light)" }}>@{item.sellerHandle}</span>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{ background: "var(--cream)", minHeight: "100vh" }}>
      ...
      <style>{`
        ...
        .apparel-featured-card { display: flex; flex-direction: column; height: 100%; }
        .apparel-featured-image { position: relative; width: 100%; aspect-ratio: 4 / 3; overflow: hidden; flex-shrink: 0; isolation: isolate; }
        .apparel-featured-image-element { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; display: block; }
        .apparel-featured-content { display: flex; flex-direction: column; flex: 1; min-height: 0; }
        .apparel-featured-price-row { display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-top: auto; flex-wrap: wrap; }
        .apparel-featured-price { min-width: 0; overflow-wrap: anywhere; }
        ...
        @media (max-width: 640px) { .category-photo-hero { height: 130px; } .category-photo-hero-text { padding-top: 0; padding-bottom: 0; } .category-photo-hero-eyebrow { margin-bottom: 0; } .category-photo-hero-description { margin-top: 0; } .apparel-grid { column-gap: 10px; row-gap: 14px; } }
